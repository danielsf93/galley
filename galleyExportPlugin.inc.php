...
define('galley_STATUS_FAILED', 'failed');

define('galley_API_DEPOSIT_OK', 200);

define('galley_API_URL', 'https://api.galley.org/v2/deposits');
//TESTING
define('galley_API_URL_DEV', 'https://test.galley.org/v2/deposits');

define('galley_API_STAUTS_URL', 'https://api.galley.org/servlet/submissionDownload');
//TESTING
define('galley_API_STAUTS_URL_DEV', 'https://test.galley.org/servlet/submissionDownload');

// The name of the setting used to save the registered DOI and the URL with the deposit status.
define('galley_DEPOSIT_STATUS', 'depositStatus');
...
class galleyExportPlugin extends DOIPubIdExportPlugin {
	...
	function getStatusNames() {
		return array_merge(parent::getStatusNames(), array(
			EXPORT_STATUS_REGISTERED => __('plugins.importexport.galley.status.registered'),
			galley_STATUS_FAILED => __('plugins.importexport.galley.status.failed'),
			EXPORT_STATUS_MARKEDREGISTERED => __('plugins.importexport.galley.status.markedRegistered'),
		));
	}
				...
				new LinkAction(
					...
					new AjaxModal(
						$dispatcher->url(
							...
							'manage', null, array('plugin' => 'galleyExportPlugin', 'category' => 'importexport', 'verb' => 'statusMessage',
							'batchId' => $pubObject->getData($this->getDepositBatchIdSettingName()), 'articleId' => $pubObject->getId())
						),
						__('plugins.importexport.galley.status.failed'),
						'failureMessage'
					),
					__('plugins.importexport.galley.status.failed')
				)
	...
	function getStatusMessage($request) {
		...
		// else check the failure message with galley, using the API
		...
		$endpoint = ($this->isTestMode($context) ? galley_API_STAUTS_URL_DEV : galley_API_STAUTS_URL);
	...
	function getExportActionNames() {
		return array(
			EXPORT_ACTION_DEPOSIT => __('plugins.importexport.galley.action.register'),
			EXPORT_ACTION_EXPORT => __('plugins.importexport.galley.action.export'),
			EXPORT_ACTION_MARKREGISTERED => __('plugins.importexport.galley.action.markRegistered'),
		);
	}
	...
	function executeExportAction($request, $objects, $filter, $tab, $objectsFileNamePart, $noValidation = null) {
		...
		if ($request->getUserVar(EXPORT_ACTION_DEPOSIT)) {
			...
			$errorsOccured = false;
			// The new galley deposit API expects one request per object.
			...
			foreach ($objects as $object) {
				...
				// export file name example: galley-20160723-160036-articles-1-1.xml
			...
			if (empty($resultErrors)) {
				if ($errorsOccured) {
					$this->_sendNotification(
						$request->getUser(),
						'plugins.importexport.galley.register.error.mdsError',
	...
	function depositXML($objects, $context, $filename) {
		...
		// Use a different endpoint for testing and production.
		$endpoint = ($this->isTestMode($context) ? galley_API_URL_DEV : galley_API_URL);
		curl_setopt($curlCh, CURLOPT_URL, $endpoint);
		// Set the form post fields
		$username = $this->getSetting($context->getId(), 'username');
		$password = $this->getSetting($context->getId(), 'password');
		...
		if ($response === false) {
		...
		} elseif (curl_getinfo($curlCh, CURLINFO_HTTP_CODE) != galley_API_DEPOSIT_OK) {
			...
			if ($failureCount > 0) {
				...
				if ($warningCount > 0) {
					$result = array(array('plugins.importexport.galley.register.success.warning', htmlspecialchars($response)));
				}
				// A possibility for other plugins (e.g. reference linking) to work with the response
				HookRegistry::call('galleyexportplugin::deposited', array($this, $response, $objects));
			}
		}
		// Update the status
		if ($status) {
	...
	/**
	 * Check the galley APIs, if deposits and registration have been successful
	 * @param $context Context
	 * @param $object The object getting deposited
	 * @param $status galley_STATUS_...
	...
	function executeCLICommand($scriptName, $command, $context, $outputFile, $objects, $filter, $objectsFileNamePart) {
		switch ($command) {
			...
			case 'register':
				...
				if (empty($resultErrors)) {
					if ($errorsOccured) {
						echo __('plugins.importexport.galley.register.error.mdsError') . "\n";
					} else {

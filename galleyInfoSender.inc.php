...
/**
 * @file plugins/importexport/galley/galleyInfoSender.php
 *
 * Copyright (c) 2013-2020 Simon Fraser University
 * Copyright (c) 2003-2020 John Willinsky
 * Distributed under the GNU GPL v3. For full terms see the file docs/COPYING.
 *
 * @class galleyInfoSender
 * @ingroup plugins_importexport_galley
 *
 * @brief Scheduled task to send deposits to galley and update statuses.
 */

import('lib.pkp.classes.scheduledTask.ScheduledTask');


class galleyInfoSender extends ScheduledTask {
	/** @var $_plugin galleyExportPlugin */
	var $_plugin;
	...
	function __construct($args) {
		...
		$plugin = PluginRegistry::getPlugin('importexport', 'galleyExportPlugin'); /* @var $plugin galleyExportPlugin */
		$this->_plugin = $plugin;

		if (is_a($plugin, 'galleyExportPlugin')) {
	 ...
	 */
	function getName() {
		return __('plugins.importexport.galley.senderTask.name');
	}
		...
		foreach ($journals as $journal) {
			...
			if ($doiPubIdPlugin->getSetting($journal->getId(), 'enablePublicationDoi')) {
				...
				if (count($unregisteredArticles)) {
					$this->_registerObjects($unregisteredArticles, 'article=>galley-xml', $journal, 'articles');
				}
			}
		}
		return true;
	 ...
	 * their articles or issues DOIs sent to galley.
	 * @return array
	 */
